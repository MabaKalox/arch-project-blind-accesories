...
void IRAM_ATTR onTimer() {
    ...
    if (GameAudioGlobalObject->CurrentWav != 0) {
        if (GameAudioGlobalObject->CurrentWav->Completed == false) {
            dacWrite(GameAudioGlobalObject->DacPin,
                     GameAudioGlobalObject->CurrentWav->Data[GameAudioGlobalObject->CurrentWav->DataIdx]);                                                            // write out the data
            GameAudioGlobalObject->CurrentWav->DataIdx++;
            if (GameAudioGlobalObject->CurrentWav->DataIdx >=
                GameAudioGlobalObject->CurrentWav->DataSize)  // if that was the last data byte...
...
Game_Audio_Class::Game_Audio_Class(uint8_t DacPin, uint8_t TimerNo) {
    ...
    timerAlarmWrite(timer, ESP32_F_CPU / AUDIO_INTERRUPT_PRESCALER / MIN_SAMPLERATE,
                    true); // use min rate until a wav file is played
    timerAlarmEnable(timer);                              // enable
    GameAudioGlobalObject = this;                        // set global variable to use in ISR
    this->DacPin = DacPin;
    dacWrite(DacPin,
             0x7f);                                // Set speaker to mid point to stop any clicks during sample playback
}
...
int Game_Audio_Class::PlayWav(Game_Audio_Wav_Class *Wav, bool interruptCurrent, float sampleRateMultiplier) {
    ...
    if (interruptCurrent == false && GameAudioGlobalObject->CurrentWav !=
                                     NULL) // if it is NULL a wav has not been played yet and we cannot check values
    {
        if (GameAudioGlobalObject->CurrentWav->Completed == false)
            return 0;
    }
    ...
    if (sampleRateMultiplier != 1.0) {
        ...
        if (newSampleRate > MAX_SAMPLERATE || newSampleRate < MIN_SAMPLERATE) {
            timerAlarmWrite(timer, ESP32_F_CPU / AUDIO_INTERRUPT_PRESCALER / Wav->SampleRate,
                            true);  // set the interrupt rate to the sample rate
        } else {
            timerAlarmWrite(timer, ESP32_F_CPU / AUDIO_INTERRUPT_PRESCALER / newSampleRate,
                            true);  // set the interrupt rate to the sample rate
        }
...
Game_Audio_Wav_Class::Game_Audio_Wav_Class(unsigned char *WavData) {
    ...
    DataSize = (WavData[WAV_FILESIZE_H] * 65536) + (WavData[WAV_FILESIZE_M] * 256) + WavData[WAV_FILESIZE_L] +
               AUDIO_DATA_START;
    Data = WavData;
    //Count=0;
